...
void testCase_1()
{
    ...
    *(int *)value.data = 5;
    ...
    ts->setIterator();
    /*
    Aggregate *ag1=new Aggregate(ts,b1,SUM);
    
    char newb[100],temp3[100];
    memset(newb,0,100);
    while(ag1->getNextTuple(newb)!=RM_EOF)
    {
    	 cout<<"SUM"<<*(int*)&newb<<endl;
    	 memset(newb,0,100);
    }


    vector<Attribute> att;
    ag1->getAttributes(att);

    cout<<att[0].name<<endl;
*/
...
/*
    while(is->getNextTuple(a1)!=RM_EOF)
    {
    		d1=is->getNextTuple(a1);
    	    memset(a2,0,100);
    	    memcpy(a2,a1,4);
    	    cout<<"SHABBA "<<*(int*)&a2<<endl;
    	    memset(a1,0,100);
    }
*/
...
void testCase_4()
{
    ...
    /*
    while(leftIn->getNextTuple(nbuff)!=RM_EOF)
    {
    	memset(nbuff2,0,100);
    	memcpy(nbuff2,nbuff+4,4);
    	cout<<"LEFT"<<*(int*)&nbuff2<<endl;
    }

    memset(nbuff2,0,100);
    cout<<endl;
    while(rightIn->getNextTuple(nbuff)!=RM_EOF)
     {
     	memset(nbuff2,0,100);
     	memcpy(nbuff2,nbuff,4);
     	cout<<"RIGHT"<<*(int*)&nbuff2<<endl;
     }
    */
    ...
    /*
    int a1=nljoin.getNextTuple(data);
    cout<<"----"<<a1<<endl;
    a1=nljoin.getNextTuple(data);
    cout<<"----"<<a1<<endl;
    a1=nljoin.getNextTuple(data);
    cout<<"----"<<a1<<endl;
*/
...
/*
    int a3=leftIn->getNextTuple(nbuff);
    memcpy(nbuff2,nbuff+12,1);
    cout<<"A--> "<<nbuff2<<endl;
    a3=leftIn->getNextTuple(nbuff);
        memcpy(nbuff2,nbuff+12,1);
        cout<<"A--> "<<nbuff2<<endl;
    /*
    a3=inljoin.getNextTuple(nbuff);
    memcpy(nbuff2,nbuff+12,1);
    cout<<"A--> "<<nbuff2<<endl;
    a3=inljoin.getNextTuple(nbuff);
       memcpy(nbuff2,nbuff+12,1);
